...
pygame.display.set_caption('Droppie')
...
# background
pine1_img = pygame.image.load('images/Background/pine1.png').convert()
pine1_img.set_colorkey(WHITE)
pine2_img = pygame.image.load('images/Background/pine2.png').convert()
pine2_img.set_colorkey(WHITE)
mountain_img = pygame.image.load('images/Background/mountain.png').convert()
mountain_img.set_colorkey(BLACK)
sky_img = pygame.image.load('images/Background/sky_cloud.png').convert_alpha()
# store tiles in a list
img_list = []
for x in range(TILE_TYPES):
    img = pygame.image.load(f'images/Tile/{x}.png').convert()
...
grenade_img.set_colorkey(BLACK)
# pick up boxes
...
class Character(pygame.sprite.Sprite):
    def __init__(self, char_type, x, y, scale, speed, ammo, grenades):
        ...
        animation_types = ['stay_animation', 'walk_animation', 'jump_animation', 'death_animation']
        for animation in animation_types:
            ...
            for i in range(num_of_frames):
                img = pygame.image.load(f'images/{self.char_type}/{animation}/p1_{i}.png').convert()
                img.set_colorkey(BLACK)
        ...
        for y, row in enumerate(data):
            for x, tile in enumerate(row):
                if tile >= 0:
                    ...
                    #     water_group.add(water)
                    ...
                    elif tile == 17:  # create grenade box
                        ...
                        enemy = Character('enemy', x * TILE_SIZE, y * TILE_SIZE, 1.65, 2, 20, 0)
...
# create buttons
start_button = button.Button(SCREEN_WIDTH // 2 - start_img.get_width(), SCREEN_HEIGHT // 2 - 100, start_img, 2)
exit_button = button.Button(SCREEN_WIDTH // 2 - exit_img.get_width(), SCREEN_HEIGHT // 2, exit_img, 2)
restart_button = button.Button(SCREEN_WIDTH // 2 - restart_img.get_width(), SCREEN_HEIGHT // 2, restart_img, 2)
...
while run:
    ...
    if start_game == False:
        ...
        # show ammo
        draw_text('WATER BULLETS: ', font, WHITE, 10, 35)
        for x in range(player.ammo):
            screen.blit(bullet_img, (170 + (x * 10), 35))
        # show grenades
        draw_text('WATER GRENADES: ', font, WHITE, 10, 60)
        for x in range(player.grenades):
            screen.blit(grenade_img, (180 + (x * 15), 60))
